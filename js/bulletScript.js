var bulletWidth = 40, //Projectile Width
    ...
    grimEnemyFric = 0.8, //Grim reaper friction
    grimEnemyVel = 0, //Enemy velocity
    grimEnemyx = 280, //Enemy x
    grimEnemyy = 300, //Enemy y
    grimEnemySpeed = 4, //Enemey speed
...
function bulletScript(HEIGHT) {
    ...
    /**/ //Drawing enemys
    enemyImg = document.getElementById('grim');
        /**/ //If enemy is not dead display the image
    if (enemyHp != 0){
        grimReaper = createImage(enemyImg, enemyx, enemyy, enemyW, enemyH);
    }
        /**/
    /**/
...
function grimBulletScript() {
    ...
    if (bulletyc < HEIGHT) {
        ...
        if (canShoot == true) {
            if (initpos == 1){
                isBackwards = false;
                bulletxc = grimEnemyx; //Setting start position
                bulletyc = grimEnemyy; //Setting position
                initpos = 0; //Setting init value
                shootTimeModifier = Math.floor(Math.random() * 7) + 1;
            }
            bulletSpeed = 6; //Bullet speed set to 6 regardless of what has been set before
    ...
    /**/ //Setting value to change direction
    if (left){
        if (grimEnemyVel > -grimEnemySpeed){
            grimEnemyVel--;
        }
    ...
    /**/ //Enemy friction and moves enemy
    grimEnemyVel *= grimEnemyFric;
    grimEnemyx += grimEnemyVel;
    /**/
    ...
    grimReaperImg = document.getElementById('grim');
        /**/ //If enemy is not dead display the image
    if (enemyHp != 0){
        grimReaper = createImage(grimReaperImg, grimEnemyx, grimEnemyy, grimEnemyW, grimEnemyH);
    }
        /**/
    /**/
    ...
    var bulletx = bulletxc + (bulletWidth - 32);
    var bullety = bulletyc + bulletHeight;
    var combineEx = grimEnemyx + grimEnemyW;
    var combineEy = grimEnemyy + grimEnemyH;
    /**/

    /**/ //Bullet collision with enemy
    if ((bulletyc <= combineEy && grimEnemyy <= bullety) && (grimEnemyx <= bulletx && combineEx >= bulletxc)) {
        if (isBackwards){
